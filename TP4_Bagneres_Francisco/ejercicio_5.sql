...
(87164,"Legolas","Bosque Negro","Krause y VIllavieja",3514,"3971-544444",1948,"historial de cardiopatias"),
...
-- a
insert into PACIENTE(nss, nombre, apellido, domicilio, codigo_postal, telefono, nro_historial_clinico, observaciones) values
(22558, "Antonio", "Magallanes", "Dos Pindo 260", 3640, null, 1932, null);

-- b
update PACIENTE set telefono="3745-589174" where nro_historial_clinico = 1932;

-- c
select concat(apellido,", ", nombre) as paciente, observaciones as obersevacion_registrada from PACIENTE;

-- d
select concat(paciente.apellido,", ", paciente.nombre) as paciente, medico.especialidad from ingreso
...
-- e
delete from medico where matricula not in(select distinct matricula_medico from ingreso);

-- f
select nro_cama as numero_de_cama, count(*) as usos from ingreso group by nro_cama order by usos desc;

-- g
select concat(apellido,", ", nombre) as paciente, count(*) as ingresos from ingreso
inner join paciente on ingreso.nro_historial_paciente = paciente.nro_historial_clinico
group by paciente.nro_historial_clinico having count(*) >= 4;
